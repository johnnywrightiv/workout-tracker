...
export default function Navbar() {
	...
	return (
		<>
						...
						<div className="ml-auto flex items-center space-x-4">
							...
							{!isAuthenticated && (
								<Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
									...
									<SheetContent
										side="right"
										className="animate-slide-in-from-right w-[300px] sm:w-[400px]"
									>
			...
			{isAuthenticated && (
				<div
					className={`border-primary bg-background fixed bottom-0 left-0 right-0 z-10 flex flex-row justify-between space-x-2 border-t p-2 transition-transform duration-300 md:hidden ${
					...
					{[...mobileNavItems, settingsItem].map((link) => {
						...
						return (
							<Link
								...
								href={link.href}
								className={cn(
									'text-muted-foreground hover:bg-secondary/30 hover:text-card-foreground focus:ring-primary flex h-[48px] grow items-center justify-center rounded-[--radius] p-3 text-sm font-medium focus:outline-none focus:ring-1',
									{
